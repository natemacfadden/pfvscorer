...
def test_encoders_output_shape():
    b = synthetic_batch()
    assert KappaEncoder(MAX_H11, d_out=128)(b['kappa_idx'], b['kappa_v'], b['kappa_mask']).shape == (B, 128)
    assert C2Encoder(MAX_H11, d_out=128)(b['c2'], b['c2_mask']).shape == (B, 128)
    assert HEncoder(MAX_H11, d_out=128)(b['H'], b['H_row_mask'], b['H_col_mask']).shape == (B, 128)
...
def test_padding_invariance():
    ...
    pad_k, pad_h = 3, 2
    bp = dict(b)
    bp['kappa_idx']  = torch.cat([b['kappa_idx'],  torch.randint(0, MAX_H11, (B, pad_k, 3))], dim=1)
    bp['kappa_v']    = torch.cat([b['kappa_v'],    torch.randn(B, pad_k)], dim=1)
    bp['kappa_mask'] = torch.cat([b['kappa_mask'], torch.zeros(B, pad_k, dtype=torch.bool)], dim=1)
    bp['H']          = torch.cat([b['H'],          torch.randint(-3, 3, (B, pad_h, MAX_H11 - 1))], dim=1)
    bp['H_row_mask'] = torch.cat([b['H_row_mask'], torch.zeros(B, pad_h, dtype=torch.bool)], dim=1)
    with torch.no_grad():
        out = model.probs(bp)
