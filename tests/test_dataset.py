...
SAMPLE_PARQUET = os.path.join(
    os.path.dirname(DEFAULT_PARQUET), "sample_coni_pfvs.parquet"
)
# prefer the full dataset when present; fall back to the committed sample so
# these tests run (not skip) from a fresh clone
DATA_PARQUET = DEFAULT_PARQUET if os.path.exists(DEFAULT_PARQUET) else SAMPLE_PARQUET
...
@pytest.mark.skipif(
    not os.path.exists(DATA_PARQUET), reason="no parquet (full or sample) present"
)
...
def test_conditioned_count_matches_filter():
    ...
    for i in [i for i in (0, 1, 2, 100, 1000) if i < len(ds)]:
        s = ds[i]
        r = ds.df.iloc[i]
        infn = np.asarray(r.pfv_infnorm)
        rdil = np.asarray(r.pfv_reqdil)
