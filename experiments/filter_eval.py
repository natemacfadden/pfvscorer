...
def build_sample(row, Bp, Dp):
    ...
    return {
        "kappa_idx": ki,
        "kappa_v": kv,
        "c2": torch.tensor(list(row.c2), dtype=torch.long),
        "H": Ht,
        "h11": torch.tensor(int(row.h11), dtype=torch.long),
        "h21": torch.tensor(float(row.h21), dtype=torch.float32),
        "B": torch.tensor(float(Bp)),
        "dil": torch.tensor(float(Dp)),
...
def main():
    ...
    ap.add_argument(
        "--label_thresh", type=float, default=0.0, help="positive = npfvs > this"
    )
    ap.add_argument(
        "--head", type=int, default=0, help="which head to score (multi-head models)"
    )
    ...
    val_idx = (
        ckpt["val_idx"]
        if "val_idx" in ckpt
        else coni_split(len(df), a["val_frac"], a["test_frac"], a["seed"])[1]
    )
    ...
    for idx in val_idx:
        r = df.iloc[idx]
        fb = np.asarray(r.frontier_infnorm, np.int64)
        fd = np.asarray(r.frontier_dil, np.int64)
        k = int(np.argmax(fb * fd))
        samples.append(build_sample(r, int(fb[k]), int(fd[k])))
        h11s.append(int(r.h11))
        labels.append(int(r.npfvs > args.label_thresh))
    scores = []
    ...
    print(
        f"ckpt={args.ckpt}  head={args.head}  label=npfvs>{args.label_thresh:g}  "
    ...
    for hv in np.unique(h11s):
        m = h11s == hv
        print(
            f"  h11={hv:>2}: AUC={auc(scores[m], labels[m]):.3f}  "
    ...
    for frac in (0.1, 0.25, 0.5, 0.75):
        ...
        nulls_total = (~labels).sum()
        nulls_skipped = (~lab_sorted[k:]).sum() if k < len(scores) else nulls_total
        print(
            f"    top {int(frac * 100):>2}%:  recall={rec * 100:5.1f}%   nulls correctly skipped={nulls_skipped / nulls_total * 100:5.1f}%"
        )
    ...
    for target in (0.9, 0.95, 0.99):
        ...
        print(
            f"    to catch {int(target * 100)}% of positives: study {need * 100:.1f}% of conis"
        )
