...
def bs(r, B, D):
    ...
    if kc: arr = torch.tensor(kc, dtype=torch.long); ki, kv = arr[:, :3].contiguous(), arr[:, 3].float()
    else: ki = torch.zeros((0, 3), dtype=torch.long); kv = torch.zeros((0,))
    H = [list(x) for x in r.H]; Ht = torch.tensor(H, dtype=torch.long) if H else torch.zeros((0, 0), dtype=torch.long)
    return {'kappa_idx': ki, 'kappa_v': kv, 'c2': torch.tensor(list(r.c2), dtype=torch.long), 'H': Ht,
            'h11': torch.tensor(int(r.h11)), 'h21': torch.tensor(float(r.h21)),
            'B': torch.tensor(float(B)), 'dil': torch.tensor(float(D)), 'count': torch.tensor(0)}
...
def score(model, df, idxs, dev):
    ...
    samp, npf = [], []
    for i in idxs:
        r = df.iloc[i]; fb = np.asarray(r.frontier_infnorm); fd = np.asarray(r.frontier_dil)
        k = int(np.argmax(fb * fd)); samp.append(bs(r, int(fb[k]), int(fd[k]))); npf.append(int(r.npfvs))
    out = []
    for j in range(0, len(samp), 512):
        b = {k: v.to(dev) for k, v in collate(samp[j:j + 512]).items()}
        with torch.no_grad(): out.append(torch.sigmoid(model(b)).cpu().numpy())
...
def binstats(p, y, bins=BINS):
    ...
    edges = np.linspace(0, 1, bins + 1); xs, mean, std = [], [], []
    for lo, hi in zip(edges[:-1], edges[1:]):
        m = (p >= lo) & (p < hi) if hi < 1 else (p >= lo) & (p <= hi)
        n = int(m.sum())
        if n < 1: continue
        k = int(y[m].sum()); xs.append(p[m].mean())
        mean.append((k + 1) / (n + 2)); std.append(float(_Beta.std(k + 1, n - k + 1)))
    return np.array(xs), np.array(mean), np.array(std)


def main():
    ckpt = torch.load(CKPT, map_location='cpu', weights_only=False)
    a = ckpt['args']; dev = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for col, thr, name in heads:
        yv = (npv > thr).astype(float); yt = (npt > thr).astype(float)
        ...
        print(f"  Brier  raw={np.mean((raw_t-yt)**2):.3f} -> cal={np.mean((cal_t-yt)**2):.3f}")
        print(f"  ECE    raw={ece(raw_t,yt):.3f} -> cal={ece(cal_t,yt):.3f}")
        print(f"  raw P range [{raw_t.min():.2f},{raw_t.max():.2f}]  cal P range [{cal_t.min():.2f},{cal_t.max():.2f}]")
        print(f"  operating points (calibrated, test):")
        for tau in (0.3, 0.5, 0.7, 0.9):
            fl = cal_t > tau
            if fl.sum():
                prec = yt[fl].mean(); rec = yt[fl].sum()/max(yt.sum(),1)
                print(f"    P>{tau}: flag {fl.mean()*100:4.0f}%  precision {prec*100:4.0f}%  recall {rec*100:4.0f}%")
        rng = np.random.default_rng(0)
        xb, mb, sb = binstats(cal_t, yt)
        ax[col].scatter(cal_t, yt + rng.uniform(-.03, .03, len(yt)), s=5, alpha=0.05, color='k', zorder=1)
        ax[col].plot([0, 1], [0, 1], 'k:', lw=1, zorder=2, label='perfect')
        ax[col].errorbar(xb, mb, yerr=sb, fmt='o', ms=5, color='#1f77b4', ecolor='#1f77b4',
                         elinewidth=1.5, capsize=3, zorder=4, label='bin rate')
        ax[col].set_title(f'head {col}: {name} (test data)')
        ax[col].set_xlabel(f'model prob for >{thr} PFVs'); ax[col].set_ylabel(f'P(>{thr} PFVs | model prediction)')
        ax[col].set_xlim(-0.04, 1.04); ax[col].set_ylim(-0.1, 1.12)
        ax[col].legend(loc='lower right', bbox_to_anchor=(1.0, 0.1))
    fig.tight_layout(); fig.savefig('experiments/calibration.png', dpi=120)
